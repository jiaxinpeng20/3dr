//mpi communications for complex data structures(STL)
#include<numeric>
#include<Eigen/Eigen>
...
namespace XSFM
{
    namespace Communication
    {
        ...
        void Gatherv(boost::mpi::communicator& world, std::vector<std::pair<std::pair<T, T>, std::vector<std::pair<T, T>>>>& inData, std::vector<std::pair<std::pair<T, T>, std::vector<std::pair<T, T>>>>& outData, int root)
        {
            ...
            std::cout<<"xxxxxxxxxxxxxxxxxxxxxxxxxxx  "<<root<<", "<<world.rank()<<std::endl;
            
            
                          
            std::cout<<"start gather process..."<<std::endl;
            ...
            boost::mpi::gather(world, itemCount, gatherItemCountList, root);//gather all item counts    
                ...
                std::cout<<gatherItemDataCountList.size()<<std::endl;
                //Gather the content of complex data from all computing nodes
                std::vector<int> gatherDataCountList;
                boost::mpi::all_gather(world, localDataCount, gatherDataCountList);
                int globalDataCount = std::accumulate(gatherDataCountList.begin(), gatherDataCountList.end(), 0);
                
                
                for(int s = 0; s < gatherDataCountList.size(); s++)
                {
                    std::cout<<gatherDataCountList.at(s)<<", ";
                }
                std::cout<<std::endl;
                
                std::cout<<"$$$$$$$$$$$$$$$$$$$$$$$$$ "<<localDataCount<<", "<<globalDataCount<<std::endl;
            ...
            {  
                std::cout<<gatherSerializedData.size()<<std::endl;
